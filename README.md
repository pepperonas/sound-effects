...
[![Sounds](https://img.shields.io/badge/sounds-598-ff69b4)](output/)
[![Categories](https://img.shields.io/badge/categories-36-blueviolet)](#-categories)
...
[![Build time](https://img.shields.io/badge/full%20build-~95s-lightgrey)](#-usage)
...
[![DJ one-shots](https://img.shields.io/badge/DJ%20one--shots-200-ff5722)](#-dj-one-shots-short-enough-to-trigger-by-hand)
[![Trigger length](https://img.shields.io/badge/DJ%20samples-%E2%89%A41.2s%20%C2%B7%20%C3%B8%200.56s-ff5722)](#-dj-one-shots-short-enough-to-trigger-by-hand)
...
- 🧰 **598 ready-to-use sounds** across **36 categories** — UI interactions, music production **and** a DJ sample deck
...
- 🎚️ **A 200-piece DJ one-shot kit** — orchestra hits, turntable scratches, air horns, tape stops, drum fills, 808 slides, glitches, percussion, crowds and vocal shouts, none longer than 1.2 s so they trigger on the beat
...
a sample deck at once. **Nothing runs past 1.2 s and the average is 0.56 s** —
...
| 🥁 **fills** | tom, snare, hat and clap fills; rolls, flams, drags | 20 |
| 🪘 **perc** | shaker, clave, woodblock, conga, tambourine, triangle | 20 |
| 🔊 **subs** | 808s, pitch slides, sub drops, reese growls, stabs | 20 |
| 📟 **glitch** | stutter, bit-crush, static, tape wow, CD skip, telephone | 20 |
| 👏 **crowd** | applause, ovation, cheers, whistles, chants, boos | 20 |
...
**fills** · `fill_toms_down` · `fill_snare_accel` · `fill_hat_roll` · `fill_half_bar` · `fill_reverse`
**perc** · `perc_shaker` · `perc_clave` · `perc_woodblock` · `perc_conga_hi` · `perc_tambourine` · `perc_triangle`
**subs** · `sub_808` · `sub_slide_up` · `sub_fall` · `sub_reese` · `sub_donk` · `sub_wobble`
**glitch** · `glitch_stutter` · `glitch_bitcrush` · `glitch_tape_wow` · `glitch_cd_skip` · `glitch_telephone`
**crowd** · `crowd_applause` · `crowd_ovation` · `crowd_cheer` · `crowd_whistle` · `crowd_chant_3`
...
python3 build.py -g dj                 # just the DJ one-shots (~20 s)
...
```bash
python3 -m unittest discover tests -v      # 126 tests, stdlib only, ~19 s
...
| `tests/test_dj.py` | The DJ constraints: **nothing over 1.2 s**, no dead head, no DC offset, 154 of 200 punching inside 30 ms — and **all 80** in the four categories that are hits rather than builds — plus that a scratch's platter speed really sets its pitch, its crossfader really silences, and "eh" really does put energy where "ah" does not |
